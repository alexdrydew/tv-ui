...
mod tests {
    ...
    fn create_test_app() -> (tauri::App<MockRuntime>, WebviewWindow<MockRuntime>) {
        let app = mock_builder()
            ...
            .invoke_handler(tauri::generate_handler![
                ...
                commands::get_app_state,
                commands::kill_app,
                commands::create_app_config // Added for testing
            ])
    ...
    // --- Tests for create_app_config ---

    #[tokio::test]
    async fn test_create_app_config_new_file() {
        let (app, webview) = create_test_app();
        let temp_dir = tempfile::tempdir().expect("Failed to create temp dir");
        let config_path = temp_dir
            .path()
            .join("test_config_new.json")
            .to_str()
            .unwrap()
            .to_string();
        ...
        let new_config = AppConfig {
            ...
            icon: "new_icon.png".to_string(),
            launch_command: "new_command".to_string(),
        };

        let response = tauri::test::get_ipc_response::<WebviewWindow<MockRuntime>>(
            &webview,
            tauri::webview::InvokeRequest {
                cmd: "create_app_config".into(),
                callback: ipc::CallbackFn(0),
                error: ipc::CallbackFn(1),
                url: "http://tauri.localhost".parse().unwrap(),
                body: ipc::InvokeBody::Json(
                    serde_json::json!({ "newConfig": new_config, "configPath": config_path }),
                ),
                headers: Default::default(),
                invoke_key: INVOKE_KEY.to_string(),
            },
        )
        .expect("Failed to get IPC response for create_app_config (new file)");

        let _: () = response
            .deserialize()
            .expect("Failed to deserialize create_app_config response (expected null)");

        // Verify file content
        let content = fs::read_to_string(temp_dir.path().join("test_config_new.json"))
            .expect("Failed to read config file after creation");
        let configs: Vec<AppConfig> =
            serde_json::from_str(&content).expect("Failed to parse created config file");
        assert_eq!(configs.len(), 1);
        assert_eq!(configs[0], new_config);

        // TODO: Verify event emission if possible
        drop(app); // Keep app alive until here
    }

    #[tokio::test]
    async fn test_create_app_config_existing_file() {
        let (app, webview) = create_test_app();
        let temp_file = NamedTempFile::new().expect("Failed to create temp file");
        ...
        let new_config = AppConfig {
            ...
            name: "App Two".to_string(),
            icon: "icon2.png".to_string(),
            launch_command: "command2".to_string(),
        };
        ...
        let response = tauri::test::get_ipc_response::<WebviewWindow<MockRuntime>>(
            ...
            tauri::webview::InvokeRequest {
                ...
                url: "http://tauri.localhost".parse().unwrap(),
                body: ipc::InvokeBody::Json(
                    serde_json::json!({ "newConfig": new_config, "configPath": temp_file.path() }),
                ),
                headers: Default::default(),
                invoke_key: INVOKE_KEY.to_string(),
            },
        )
        .expect("Failed to get IPC response for create_app_config (existing file)");
        ...
        let content = fs::read_to_string(temp_file.path())
            .expect("Failed to read config file after adding");
        let configs: Vec<AppConfig> =
            serde_json::from_str(&content).expect("Failed to parse updated config file");
        assert_eq!(configs.len(), 2);
        assert!(configs.iter().any(|c| c.id == "app1"));
        assert!(configs.iter().any(|c| c.id == "app2"));
    ...
    #[tokio::test]
    async fn test_create_app_config_duplicate_id() {
        let (_app, webview) = create_test_app();
        let temp_file = NamedTempFile::new().expect("Failed to create temp file");
        let initial_configs = vec![AppConfig {
            id: "app1".to_string(),
        ...
        let duplicate_config = AppConfig {
            id: "app1".to_string(), // Same ID
            name: "App One Duplicate".to_string(),
            icon: "icon1_dup.png".to_string(),
            launch_command: "command1_dup".to_string(),
        };
        ...
        let result = tauri::test::get_ipc_response::<WebviewWindow<MockRuntime>>(
            ...
            tauri::webview::InvokeRequest {
                ...
                error: ipc::CallbackFn(1),
                url: "http://tauri.localhost".parse().unwrap(),
                body: ipc::InvokeBody::Json(serde_json::json!({
                    "newConfig": duplicate_config,
                    "configPath": temp_file.path()
                })),
                headers: Default::default(),
                invoke_key: INVOKE_KEY.to_string(),
            },
        ...
        let err_msg = err_val.as_str().expect("Error payload should be a string");
        assert!(
            err_msg.contains("Config with ID 'app1' already exists."),
            "Unexpected error message: {}",
            err_msg
        );
        ...
        let content = fs::read_to_string(temp_file.path())
            .expect("Failed to read config file after duplicate attempt");
        let configs: Vec<AppConfig> =
            serde_json::from_str(&content).expect("Failed to parse original config file");
        assert_eq!(configs.len(), 1);
        assert_eq!(configs[0], initial_configs[0]);
    }
}
