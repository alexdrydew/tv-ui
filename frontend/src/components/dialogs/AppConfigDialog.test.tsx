...
describe("AppConfigDialog", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(applicationApi.upsertAppConfig).mockResolvedValue(undefined);
  });
  ...
  it("[Add Mode] calls upsertAppConfig with new ID and correct data on successful submission", async () => {
    ...
    expect(applicationApi.upsertAppConfig).toHaveBeenCalledTimes(1);
    expect(applicationApi.upsertAppConfig).toHaveBeenCalledWith(
      {
        id: "mock-nanoid",
        name: "My Test App",
        icon: "/path/icon.png",
  ...
  it("[Add Mode] shows error toast and does not close dialog on failed submission", async () => {
    ...
    vi.mocked(applicationApi.upsertAppConfig).mockRejectedValue(errorMessage);
  ...
  it("[Edit Mode] renders the dialog with correct title and pre-filled fields", () => {
    ...
    expect(screen.getByLabelText("App Name")).toHaveValue(appToEditData.name);
    expect(screen.getByLabelText(/Icon Path \(Optional\)/i)).toHaveValue(
      appToEditData.icon,
    );
    expect(screen.getByLabelText("Launch Command")).toHaveValue(
